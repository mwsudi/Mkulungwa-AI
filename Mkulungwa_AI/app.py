...
if os.path.exists(f"{code}.csv"):
    ...
    if st.button("🎯 RUN MASTER AI"):
        ...
        avg_hc = h_home["HC"].mean() if "HC" in h_home.columns else 5
        avg_ac = a_away["AC"].mean() if "AC" in a_away.columns else 4
        ...
        <div class='advice'>
        ✅ SAFE PICK:
        <b>{goal_pick}</b> linaonekana salama zaidi.
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class='advice'>
        🔥 BTTS Probability:
        <b>{prob_btts*100:.1f}%</b>
        </div>
        """, unsafe_allow_html=True)

else:

    st.warning("Refresh database kwanza.")import streamlit as st
import pandas as pd
import numpy as np
import requests
import os

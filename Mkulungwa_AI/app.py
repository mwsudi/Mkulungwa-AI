...
st.markdown(
    "<p style='text-align:center;color:#8b949e;'>ELITE FOOTBALL AI ENGINE v7.0 ULTRA PRO</p>",
    unsafe_allow_html=True
)
...
if code in st.session_state.data_cache:
    ...
    injury_impact = st.selectbox(
        "🚑 Key Player Missing?",
        [
            "NO",
            "HOME STRIKER OUT",
            "AWAY STRIKER OUT",
            "BOTH TEAMS MISSING ATTACKER"
        ]
    )
    ...
    if st.button("🎯 RUN AI ANALYSIS"):
        ...
        # REVENGE FILTER
        # =========================================
        last_home_conceded = h_data["FTAG"].iloc[-1]
        last_away_conceded = a_data["FTHG"].iloc[-1]

        if last_home_conceded >= 3:
            prob_o25 -= 0.05

        if last_away_conceded >= 3:
            prob_o25 -= 0.05

        # =========================================
        # LIVE PLAYER IMPACT
        # =========================================
        if injury_impact == "HOME STRIKER OUT":

            home_xg -= 0.35
            prob_o25 -= 0.05

        elif injury_impact == "AWAY STRIKER OUT":

            away_xg -= 0.35
            prob_o25 -= 0.05

        elif injury_impact == "BOTH TEAMS MISSING ATTACKER":
        ...
        # VOLATILITY SCORE
        # =========================================
        home_volatility = np.std(h_data["FTHG"])
        away_volatility = np.std(a_data["FTAG"])

        volatility_alert = False

        if home_volatility > 1.8 or away_volatility > 1.8:
        ...
        if (h_data["FTHG"].iloc[-1] + h_data["FTAG"].iloc[-1]) > 2:
            score += 1
        ...
        with r1:
            ...
            st.markdown(
                ...
                <div class='big-val'>{goal_text}</div>
                <p class='small'>{prob_o25*100:.1f}% Prob</p>
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        with r2:
            ...
            st.markdown(
                ...
                <div class='big-val'>{btts_text}</div>
                <p class='small'>{prob_btts*100:.1f}% Prob</p>
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        with r3:
            ...
            st.markdown(
                ...
                <div class='big-val'>{corner_text}</div>
        ...
        if draw_risk < 0.22 and total_xg < 2.4:
            ...
            fb = "⛔ FINAL AI BET: NO VALUE (SKIP)"

        elif (
            score >= 7
            and edge > 5
            and not volatility_alert
            and intensity >= 20
        ):
        ...
        # =========================================
        if quality < 70:
            fb = "⛔ LOW QUALITY MATCH"

        if edge < 0:
            fb = "⛔ NEGATIVE VALUE MATCH"

        if prob_o25 > 0.80 and prob_btts < 0.45:
            fb = "⚠️ FAKE OVER DETECTED"

        if volatility_alert and score < 7:
            fb = "⚠️ UNSTABLE MATCH - SKIP"
        ...
        if volatility_alert:
            st.warning(
                "⚠️ INCONSISTENT TEAM DETECTED — Match inaweza kuwa unpredictable."
            )
...
"""
    ...
    st.warning("⚠️ Database empty. Click REFRESH DATABASE.")

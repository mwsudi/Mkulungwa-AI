    ...
    if st.button("EXECUTE ANTI-BOOKIE ANALYSIS"):
        try:
            ...
            for i, (l, v) in enumerate(signals):
                ...
                cols[i].markdown(f'<div style="border:2px solid {clr};padding:15px;border-radius:12px;text-align:center;background:#111;">{l}<br><h2 style="color:{clr};">{res:.1f}%</h2></div>', unsafe_allow_html=True)
